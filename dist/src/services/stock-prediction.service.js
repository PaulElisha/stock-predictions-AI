...
class StockPredictionService {
    fetchStockData = async (param) => {
        ...
        try {
            const stockData = await Promise.all(param.tickersArr.map(async (ticker) => {
                const response = await axios.get(`${polygonWorkerUrl}?ticker=${ticker}&startDate=${dates.startDate}&endDate=${dates.endDate}`);
                if (!response.status) {
            ...
            if (error instanceof Error) {
                ...
                throw new InternalServerError(error.message, HTTP_STATUS.INTERNAL_SERVER_ERROR, ErrorCode.INTERNAL_SERVER_ERROR);
            }
            throw error;
        }
    };
    fetchReport = async (stockData) => {
        try {
            ...
            if (status !== 200) {
                throw new BadRequestExceptionError("Worker Error", HTTP_STATUS.BAD_REQUEST, ErrorCode.RESOURCE_NOT_FOUND);
            }
            const data = response.data;
            return data;
        }
